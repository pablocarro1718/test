...
- **Símbolos correctos para Yahoo (¡crítico para el NAV!):** el `yfinance_symbol` en `SYMBOLS` debe apuntar al MISMO instrumento y escala de precio que las operaciones, o el NAV se distorsiona. Casos: `SQ`→`XYZ` (rebranding de Block); `CSPX`→`IUSA.AS` (el ISIN real IE0031442068 es la versión Dist en EUR de Ámsterdam ~€26-65, NO `CSPX.L` Acc en USD ~$720 — este bug sobrevaloraba CSPX ~10× en 2020-2025). `MAS` (Masmovil) y `TWTR` (Twitter) están deslistados (OPAs) → sin histórico → proxy de precio medio en su tramo (viejo, pequeño). Al añadir/valorar un activo, comprobar que el precio de Yahoo casa con `price_original` de las operaciones.
